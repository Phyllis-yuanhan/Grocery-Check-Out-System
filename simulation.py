...
class GroceryStoreSimulation:
    ...
    def run(self, file: TextIO) -> Dict[str, Any]:
        ...
        num_customers = 0
        total_customer = []
        initial_events = create_event_list(file)
        for events in initial_events:
            self._events.add(events)
        for a in initial_events:
            if isinstance(a, CustomerArrival):
                num_customers += 1
                total_customer.append(a.customer)
        ...
        while not self._events.is_empty():
            ...
            if isinstance(event, CheckoutCompleted):
                time1 = event.timestamp - event.customer.arrival_time
                if time1 > max_wait:
                    max_wait = time1
